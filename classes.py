		...
		if len(args) != 0:
			...
			if hasattr(args, "items"):  # Item(dict(data))
				...
				try:
					...
					raise TypeError(
						"Cannot merge data, type: {} - {}".format(type(args[0])), args[0])
				else:
		...
		for k, v in data:
			if k not in self.keys() or self[k] is None:
				...
				if not isinstance(v, type(self[k])):
					raise TypeError(
						"On key {} - types do not match: {}-{} / {}-{}".format(k, v, type(v), self[k], type(self[k])))
...
class ItemList():
	...
	def _filter_sources(self, start_thread=False):
		if start_thread:
			threading.Thread(self._filter_sources,
	...
	def map(self, func, delay, cb=None):
		...
		def map_func(func, delay, cb, idx=0):
			...
			if not self.empty():
				...
				idx += index + 1
				if not self.stop:
					delay(lambda func=func, delay=delay, cb=cb,
						  idx=idx: map_func(func, delay, cb, idx))
				return
			elif cb is not None:
				self.interrupt()
				cb(idx-1)
...
class RegroupList():
	...
	def add_element(self, sub):
		...
		if sub_id in self.data:
			...
			curset = set(item for item in current.items()
						 if item[0] not in self.merge_keys)
