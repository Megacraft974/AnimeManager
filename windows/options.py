	...
	def drawOptionsWindow(self, id):
		...
		if True:
			def findTorrent(id):
				...
				try:
					t_infos = self.tm.list(filter=torrent_managers.TorrentListFilter.DOWNLOADING, hashes=torrent_hashes)
				except torrent_managers.TorrentException:
					pass
				else:
					if len(t_infos) > 0:
						self.animeHashes[id] = set(map(lambda t: t.hash, t_infos))
			...
			def updateLoadingBar(id, bar, text):
				...
				try:
					...
					if len(torrents) == 0:
						...
						value = sum(map(lambda t: t.downloaded, torrents)) / sum(map(lambda t: t.size, torrents)) * 100
			...
			def getDateText(datefrom, dateto, broadcast):
				...
				if status == 'FINISHED':
					...
					if broadcast is not None:
						...
						tz = datetime.now().astimezone().utcoffset().seconds // 3600 # Get current UTC offset in hours
						hourDateObj = timedelta(hours=hour - 9 + tz, minutes=minute) # Compare to Japan's UTC offset (UTC+9)
