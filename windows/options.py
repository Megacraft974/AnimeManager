...
class Options:
        ...
        if True:
            ...
            def tag(id, tag):
                with self.database.get_lock():
                    self.database.set(
                        {"id": id, "tag": tag}, table="anime", get_output=False
                    )
            ...
            for relation in relations:
                ...
                if len(titles) == 1:
                    Button(
                        ...
                        fg=self.colors[
                            self.tagcolors[
                                self.database(id=rel_ids[0], table="anime").tag
                            ]
                        ],
                    ...
                    for i, rel_id in enumerate(rel_ids):
                        relList["menu"].entryconfig(
                            ...
                            foreground=self.colors[
                                self.tagcolors[
                                    self.database(id=rel_id, table="anime").tag
                                ]
                            ],
                        )
                else:
    ...
    def deleteFiles(self, id):
        ...
        with self.database.get_lock():
            self.database.set(
                {"id": id, "tag": "SEEN"}, table="anime", get_output=False
            )
