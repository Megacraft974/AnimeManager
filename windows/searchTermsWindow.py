...
class searchTermsWindow:
    def searchTermsWindow(self, id, parent=None):
        # Functions
        if True:
            def getsearchTermsManager(id):
            ...
            def draw_table(table, id):
                ...
                terms = getsearchTermsManager(id)
                ...
                for i, term in enumerate(terms):
                    ...
                    if self.closing or not self.searchTermsManager.winfo_exists():
                        return
                ...
                try:
                    if i is None:
                        self.searchTermsManager.titleLbl['text'] = "No search terms!"
                    else:
                        self.searchTermsManager.titleLbl['text'] = "Search terms"
                except TclError:
                    pass
        ...
        if True:
            ...
            if self.searchTermsManager is None or not self.searchTermsManager.winfo_exists():
                if parent is None:
                    parent = self.ddlWindow

                self.searchTermsManager = utils.RoundTopLevel(
                    parent,
                    title="Search terms",
                    minsize=size,
                ...
                self.searchTermsManager.clear()

        # Terms list
        if True:
            table = utils.ScrollableFrame(
                self.searchTermsManager, bg=self.colors['Gray2'])
